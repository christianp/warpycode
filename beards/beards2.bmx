...
Type bit
	...
	Function paint:bit(bsize,mx,my)
		b:bit=New bit
		b.angle=Rand(360)
		r!=Rnd(0,1)*bsize
		b.x=-1
		b.y=-1
		oangle=b.angle
		b.moveto(mx+Cos(b.angle)*r,my+Sin(b.angle)*r)
		b.angle=oangle
		If b.x>0 Return b
	End Function
	
	Method erase(bsize,mx,my)
		dx!=mx-x
		dy!=my-y
		d!=dx*dx+dy*dy
		If d<bsize*bsize
...
While Not (KeyHit(KEY_ESCAPE) Or AppTerminate())
	...
	Case 0
		...
		If MouseDown(1)
			For c=1 To 5
			bits.addlast bit.paint(bsize*.3,mx,my)
			Next
		EndIf	
		If MouseDown(2)
		...
		score
	Case 1
		If Not (MouseDown(1) Or MouseDown(2))
		EndIf
		
		If MouseDown(1)
			DrawRect mx,my,5,5
			template[mx/segsize,my/segsize]=dmode1
		Else
			If template[mx/segsize,my/segsize]=1
				dmode1=0
			Else
				dmode1=1
			EndIf
		...
		If MouseDown(2)
			template[mx/segsize,my/segsize]=2*dmode2
		Else
			If template[mx/segsize,my/segsize]=2
				dmode2=0
			Else
				dmode2=1
			EndIf
...
Function savetemplate()
	...
	While FileType("templates/"+fname)
		n:+1
		fname="beard"+String(n)+".txt"
	Wend
	
	f:TStream=WriteFile("templates/"+fname)
	For x=0 To xsegs-1
	For y=0 To ysegs-1
...
Function loadtemplates:TList()
	...
	While nf
		If Len(nf)>4 
			If nf[Len(nf)-4..]=".txt"
				name$=nf[..Len(nf)-4]
				f:TStream=ReadFile("templates/"+nf)
