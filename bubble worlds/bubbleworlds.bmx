...
Type circle
	...
	Method New()
		highest=pany-gheight/2
		For c:circle=EachIn circles
			If c.y<highest Then highest=c.y
		Next
	...
	Method update()
		...
		If y<starty
			extraweight#=(starty-y)*.0000007*Sqr(Abs(starty))
			startx:+(x-startx)*.01
		Else
			startx=x
			starty=y
		EndIf
		'debuglabel weight+extraweight,x,y
		vy:+weight+extraweight
	...
	Method die()
		...
		For t:thing=EachIn things
			If t.inside=Self' And t.insides.count()=1
				t.die
			EndIf
		Next
	...
	Method intersect(c2:circle)
		...
		If wallnum=0
			...
			If p.inside.connectedto(Self) Or p.inside.connectedto(c2)
				dp1#=vx*dx+vy*dy
				dp2#=c2.vx*dx+c2.vy*dy
				dv#=dp1-dp2
				If dv>.1
					volume#=dv*.5
					If volume>1 Then volume=1
					sounditem.Create(choosesound(bells),1,0,volume)
				EndIf
			EndIf
			Return
		EndIf
	...
	Method drawanchor()
		If y<starty-r
			SetColor red,green,blue
			SetLineWidth 2
			drawzoomline x,y+r,startx,starty
			SetLineWidth 1
		EndIf
	End Method
	...
	Method update()
		...
		If numinsides>1
			...
			For c:circle=EachIn insides
				tweight:+c.weight
			Next
			weight#=tweight/numinsides
		Else
			weight#=inside.weight
		EndIf
		vy:+weight
	End Method
...
Type bgrect
	...
	Method New()
		If p.vy=0 And p.vx=0 And p.x=0 And p.y=0
			x=Rnd(-gwidth/2,gwidth/2)
			y=Rnd(-gheight/2,gheight/2)
		Else
			x=panx+Rnd(-gwidth/2,gwidth/2)
			y=pany+(Sgn(p.y-pany)*.9+Rnd(-.2,.2))*gheight
		EndIf
		w=Rnd(100,300)
		h=Rnd(50,100)
		x:-w/2
	...
	Method update()
		...
		dy#=y-pany
		d=dx*dx+dy*dy
		If zoomx(x)<-50 Or zoomx(x)>gwidth+50 Or zoomy(y)>gheight+50
			bgrects.remove Self
		EndIf
	End Method
	...
	Method draw()
		...
		drawzoomrect x,y,w,h
	End Method
End Type

Function makebg()

	For bgr:bgrect=EachIn bgrects
		bgr.draw()
	Next
	
	topheight#=unzoomy(gheight)
	marker=topheight - (topheight Mod 1000)
	If marker>0 Then marker=0
	SetColor 0,0,0
	SetScale 10,10
	While zoomy(marker)>-150
		num=-marker/1000
		drawzoomtext num,unzoomx(10),marker
		marker:-1000
	Wend
	SetScale 1,1
...
Type stopwatch
	...
	Method draw()
		...
		Else
			SetColor 255,255,255
		EndIf
		DrawRect 0,n*22,diff*10,20
		DrawText txt,250,n*22
	End Method
End Type
...
Type Tdebuglabel
	...
	Method New()
		debuglabels.addlast Self
	End Method
	
	Method draw()
		SetColor 0,0,0
		drawzoomrect x,y,TextWidth(txt),TextHeight(txt)
		SetColor 255,255,255
		drawzoomtext txt,x,y
	End Method
		
End Type

Function debuglabel(txt$,x#,y#,Ab=0)
	dbl:tdebuglabel=New tdebuglabel
	dbl.txt=txt
	If ab
		dbl.x=unzoomx(x)
		dbl.y=unzoomy(y)
...
Function initgame()
	...
	bells=loadsounds("incbin::bell-",5,".ogg")
...
Function updategame()
	...
	dx#=p.x-panx
	If Abs(dx)>gwidth*.3
		panx:+dx*Abs(dx)*.00003
	EndIf
	dy#=p.y-pany-gheight*.3
	If dy<-gheight*.3
		pany:-dy*dy*.000003
	EndIf
	dy#=zoomy(p.y)-gheight*.9
	If dy>0
		pany:+dy*.03
	EndIf
	...
	If bgrects.count()<40
		New bgrect
	EndIf
	endwatch "update bgrects ("+String(bgrects.count())+")"
...
Function drawgame()
	...
	ms=MilliSecs()
	fps#=1000.0/(ms-oldms)
	debuglabel Int(fps),gwidth-50,0,1
	
	For dbl:tdebuglabel=EachIn debuglabels
		'dbl.draw
	Next
