...
Type bvalueenumerator Extends bnodeenumerator
	Method nextobject:Object()
		...
		While u<>root And u.n<t	'if node is too small
			...
			If i>0	'if u has a left sibling
				...
				If l.n>t	'borrow a key from left sibling
					...
					Next
					u.n:+1
					u.keys[0]=p.keys[i]	'insert parent's separator value at the start of u
					u.values[0]=p.values[i]
					If Not u.leaf
					...
					EndIf
					p.keys[i]=l.keys[l.n-1]	'insert left sibling's last key as new separator value
					p.values[i]=l.values[l.n-1]
