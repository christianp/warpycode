Const version$="hunkahunka"

Const screenwidth=800,screenheight=600

Global viewsize# = screenheight, screenoffsetx#=(screenwidth-screenheight)/2
Global gfxscale#=screenheight/800.0,width,height,depth,originx#,originy#
...
Function drawscaleline(x1# , y1# , x2# , y2#)
	x1 = x1 * gfxscale + screenoffsetx
	y1 = y1 * gfxscale
	x2 = x2 * gfxscale + screenoffsetx
	y2 = y2 * gfxscale
	DrawLine x1 , y1 , x2 , y2
End Function

Function drawscalepoly(poly#[])
	n = 0
	Local poly2#[Len(poly)]
	While n < Len(poly)
		poly2[n] = poly[n] * gfxscale + screenoffsetx
		poly2[n + 1] = poly[n + 1] * gfxscale
		n:+2
	Wend
	DrawPoly poly2
End Function

Function drawscaletext(txt$ , x# , y# , escalex#=1 , escaley#=1)
	SetScale gfxscale * escalex , gfxscale * escaley
	DrawText txt , x * gfxscale + screenoffsetx , y * gfxscale
	SetScale 1 , 1
End Function

Function drawscaleoval(x# , y# , w# , h#)
	x = x * gfxscale + screenoffsetx
	y = y * gfxscale
	w = w * gfxscale
	h = h * gfxscale
	DrawOval x , y , w , h
...
Type thing
	...
	Method update()
		...
		If x<0 Or x>800
			...
			If x>800 Then x=800;playbouncesound()
		...
		If Y<0 Or Y>800
			...
			If Y>800 Then y=800;playbouncesound()
			For w:wall=EachIn walls
				If w.inside(x,y)
					x=ox
					y=oy
	...
	Method draw()
		setcolours()
		drawscaleoval x-Size/2,y-Size/2,Size,Size
		SetAlpha 1
		drawscaleoval x-5,y-5,10,10
		For ot:thing=EachIn neighbours
			DrawscaleLine ot.x,ot.y,x,y
		Next
	End Method
...
Type sweep
	...
	Method draw()
		SetLineWidth 5
		SetColor red,green,blue
		DrawscaleLine sweeppoints[0].x,sweeppoints[0].y,sweeppoints[1].x,sweeppoints[1].y
		For i=0 To sweepswishlength-1
			SetAlpha 1.0-Float(i)/sweepswishlength
			DrawscaleLine sweeppoints[i*2+0].x,sweeppoints[i*2+0].y,sweeppoints[i*2+2].x,sweeppoints[i*2+2].y
			DrawscaleLine sweeppoints[i*2+1].x,sweeppoints[i*2+1].y,sweeppoints[i*2+3].x,sweeppoints[i*2+3].y
			...
			'SetAlpha 1
		Next
		DrawscaleLine sweeppoints[sweepswishlength*2].x,sweeppoints[sweepswishlength*2].y,sweeppoints[sweepswishlength*2+1].x,sweeppoints[sweepswishlength*2+1].y
		
		SetLineWidth 1
		SetAlpha .5
		drawscaleoval x - 10 , y - 10 , 20 , 20
		drawscaletext sweeppoints[0].x,x,y,1,1
		SetAlpha 1
		If an<-90 Or an>90 Then SetRotation an+180 Else SetRotation an
		DrawscaleText name,x,y,.5,.5
		SetRotation 0
...
Type wall
	...
	Method draw()
		...
		For i=0 To 2
			If drawedge[i]
				DrawscaleLine points[i*2],points[i*2+1],points[(i*2+2) Mod 6],points[(i*2+3) Mod 6]
			EndIf
		Next
		SetAlpha .3
		DrawscalePoly points
		SetAlpha 1
		SetLineWidth 1
...
Type flash
	...
	Method update()
		...
		Else
			...
			drawscaleoval x-Size,y-Size,2*Size,2*Size
			Size:-1
			If Size<=0
				flashes.Remove Self
			EndIf
...
Type scoreflash
	...
	Method update()
		...
		DrawscaleText value,x,y-30*scale*bigscale,bigscale*scale,bigscale*scale
...
Function updatemouse()
	If tabletmode Or lockmouse=0
		mx=(MouseX()-originx-screenoffsetx)/gfxscale
		my=(MouseY()-originy)/gfxscale
	Else
		mx:+(MouseX()-width/2)/gfxscale
		my:+(MouseY()-height/2)/gfxscale
		MoveMouse width/2,height/2
	EndIf
	If mx<0 Then mx=0
	If mx>800 Then mx=800
	If my<0 Then my=0
	If my > 800 Then my = 800
End Function
...
Function rungame(levelfile$="test",playername$="Player",difficulty#=1.0)
	...
	'//////////////GAME STUFF//////////////	
		...
		For s:sweep=EachIn sweeps
			...
			DrawscaleText s.name,screenwidth-200,y,.6,.6
			DrawscaleText s.displayscore,screenwidth-60,y,.6,.6
			y:+18
		Next
		...
		If timeleft>0 And gamespeed
			...
			Else
				...
				EndIf
			DrawscaleText Int(timeleft),400,0,scale,scale
		EndIf 
