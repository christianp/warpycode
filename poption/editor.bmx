...
Function runeditor()
	...
	While Not finished
		...
		'////DRAWING		
		If pointc
			DrawLine drawpoints[pointc*2-2],drawpoints[pointc*2-1],drawx,drawy
		EndIf
		...
		If selectedwall
			...
			If selectedline>=0
				...
				DrawOval selectedwall.points[a]-5,selectedwall.points[b]-5,10,10
				DrawOval selectedwall.points[c]-5,selectedwall.points[d]-5,10,10
				SetLineWidth 10
				SetAlpha selectedwall.drawedge[selectedline]*.5+.5
				DrawLine selectedwall.points[a],selectedwall.points[b],selectedwall.points[c],selectedwall.points[d]
				SetAlpha 1
				SetLineWidth 1
			EndIf
			If selectedpoint>=0
				SetAlpha .2
				DrawOval selectedwall.points[selectedpoint*2]-gridsize,selectedwall.points[selectedpoint*2+1]-gridsize,gridsize*2,gridsize*2
				SetAlpha 1
			EndIf
		EndIf
		
		SetColor 255,255,255
		If editmode=2 DrawOval drawx-2,drawy-2,5,5
		For i=1 To pointc
			DrawOval drawpoints[i*2-2]-2,drawpoints[i*2-1]-2,5,5
			If i<pointc
				DrawLine drawpoints[i*2-2],drawpoints[i*2-1],drawpoints[i*2],drawpoints[i*2+1]
			EndIf
		Next
		
		SetColor 255,255,255
		DrawText "Press S to save "+editorfilenamebutton.entrytext+".txt",0,0
