...
Type menu
	...
	Method pickhover()
		...
		If hover And hover.pickable
			...
			hover.width=80+Sin(menutick*9)*5+5
			hover.ealpha=.8
		EndIf
	End Method
...
Type tendril
	...
	Function create:tendril(sx#,sy#,ex#,ey#,width#,segments,endsegments,endlength#,sred,sgreen,sblue,salpha#,ered,egreen,eblue,ealpha#,text$,tred,tgreen,tblue,tsize#,pickable=1)
	...
	Method draw()
		...
		For c=0 To segments-1
			...
			wx#=Cos(an)*(width/2)'*(b*.5+1)
			wy#=Sin(an)*(width/2)'*(b*.5+1)
			points[c*2]=x-wx
			points[c*2+1]=y-wy
		...
		For i=0 To (segments-1)*targtick
			...
			If i<segments-1
				SetAlpha 1
				DrawLine drawpoints[0],drawpoints[1],drawpoints[2],drawpoints[3]
				DrawLine drawpoints[6],drawpoints[7],drawpoints[4],drawpoints[5]
			EndIf
		Next
		...
		If targtick=1
			...
			Local enddrawpoints#[]=[points[segments*2-2],points[segments*2-1],points[segments*2-2]+endlength*endtick,points[segments*2-1],points[segments*2-2]+endlength*endtick,points[segments*2-1]+width,points[numpoints-segments*2],points[numpoints-segments*2+1]]
			DrawPoly enddrawpoints
			SetAlpha 1
			DrawLine enddrawpoints[0],enddrawpoints[1],enddrawpoints[2],enddrawpoints[3]
			'DrawLine enddrawpoints[2],enddrawpoints[3],enddrawpoints[4],enddrawpoints[5]
			DrawLine enddrawpoints[6],enddrawpoints[7],enddrawpoints[4],enddrawpoints[5]
	
			circlestep#=180.0/endsegments
			Local endcircledraw#[endsegments*2+2]
			cx#=(enddrawpoints[2]+enddrawpoints[4])/2.0
			cy#=enddrawpoints[3]+width/2.0
			ox#=enddrawpoints[2]
			If dir>0
				oy#=enddrawpoints[3]
			Else
			...
			For i=0 To endsegments
				...
				DrawLine ox,oy,cx+x,cy+y
				ox=cx+x
				oy=cy+y
			Next
			SetAlpha ealpha
			DrawPoly endcircledraw
...
Type scorelist Extends menuitem
	Method draw()
		...
		For c=0 To nameslist.length-1
			...
			DrawText nameslist[c],200,y
			DrawText scoreslist[c],600,y
			y:+28
		Next
	End Method
End Type
...
Type botchoice Extends menuitem
	...
	Method draw()
		...
		SetScale parent.tsize,parent.tsize
		SetAlpha 1
		SetColor 255,255,255
		DrawText mybot.itssweep.name,parent.textend,parent.ey-TextHeight(mybot.itssweep.name)/2
		SetScale 1,1
	End Method
...
Type toggle Extends menuitem
	...
	Method draw()
		...
		SetAlpha .2+.8*yes
		x=parent.textend+30
		DrawOval x-20,parent.ey-20,40,40
	End Method
End Type
...
Type slider Extends menuitem
	...
	Method draw()
		...
		DrawLine l,y,r,y
		SetLineWidth 1
		DrawOval l+value*(r-l)-3,y-3,7,7
		SetAlpha .5
		DrawOval l+value*(r-l)-7,y-7,15,15
		SetAlpha 1
		SetScale parent.tsize,parent.tsize
		If isint
			DrawText Int(getvalue()),l,y
		Else
			DrawText getvalue(),l,y
		EndIf
	End Method
